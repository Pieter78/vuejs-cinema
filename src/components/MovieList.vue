...
export default {
  ...
  computed: {
    ...
    noResults() {
      const resultTimes = this.time.join(', ');
      const resultGenres = this.genre.join(', ');
      return `No results for ${resultTimes}${resultTimes.length && resultGenres.length ? ', ' : ''}${resultGenres}.`;
    },
  },
};
</script>
